...
using MiniRoslyn.Tests.CodeAnalysis.Text;
using Xunit;

namespace MiniRoslyn.Tests.CodeAnalysis
{
    public class EvaluationTests
    {
        ...
        [Fact]
        public void Evaluator_ForStatement_Reports_CannotAssign()
        {
            var text = @"
                {
                    var result = 0
        ...
        private void AssertDiagnostics(string text, string diagnostics)
        {
            ...
            Assert.True(annotatedText.Spans.Length == expectedDiagnostics.Length);
            
            for (int i = 0; i < expectedDiagnostics.Length; i++)
            {
                var expectedDiagnostic = expectedDiagnostics[i];
                var actualDiagnostic = result.Diagnostics[i].Message;
                Assert.Equal(expectedDiagnostic, actualDiagnostic);
